...
collect_quota_data() {
    ...
    while read -r _ group user fileset used soft hard grace; do
        ...
        if [[ "$grace" == "expired" ]]; then           
            grace_status="expired"                     
        elif [[ "$grace" =~ ^[0-9]+$ ]]; then         
            grace_status="warning"                     
        else                                           
            grace_status="ok"                          
        fi

        metrics+="disk_usage_details{user=\"${user}\",partition=\"${label}\",used=\"${used}\",soft=\"${soft}\",hard=\"${hard}\",percent=\"${pct}%\",grace=\"${grace_status}\"} 1"$'\n'
        metrics+="disk_usage_percent2{user=\"${user}\",partition=\"${label}\"} ${pct}"$'\n'
