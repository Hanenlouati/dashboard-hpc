...
LOG_DIR="/work/cmcc/hl06625/dashboard/serviceclass_logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/serviceclass_$(date +%Y%m%d_%H%M%S).log"

# Create header for log file
echo -e "ServiceClass\tGuarantee\tExpiration\tUsers\tApps\tProject\tQueues" > "$LOG_FILE"
...
while read -r line; do
    ...
    if [[ $line =~ ^SERVICE\ CLASS\ NAME:\ (.*) ]]; then
        ...
        if [[ -n "$current_sc" ]]; then
            ...
            echo -e "$current_sc\t$guarantee\t$expiration\t$users\t$apps\t$project\t$queues" >> "$LOG_FILE"
        fi
...
# -------------------------------
echo "DEBUG: Starting node_per_job metric collection" >> "$LOG_FILE"
all_metrics=""
declare -A job_nodes
...
while read -r line; do
    ...
    if [[ $line =~ service_classes\{service_class=\"([^\"]+)\",guarantee=\"([0-9]+)\" ]]; then
        ...
        if (( sc_guarantee > 0 )); then
            echo "DEBUG: Processing service class $sc_name (guarantee=$sc_guarantee)" >> "$LOG_FILE"
            ...
            if [[ "$jobs_output" == "No running job found" || -z "$jobs_output" ]]; then
                echo "DEBUG: No running jobs for $sc_name, pushing placeholder metric" >> "$LOG_FILE"
                metric_line="nodesc{service_class=\"$sc_name\",job_id=\"none\",user=\"none\",nb_nodes=\"0\",status=\"ok\"} 1"
                echo "$metric_line" >> "$LOG_FILE"
            ...
            while read -r jobline; do
                ...
                if [[ "$first_col" =~ ^[0-9]+$ ]]; then
                    ...
                    if [[ -n "$current_job" ]]; then
                        ...
                        job_nodes["$current_job"]=$unique_nodes
                        metric_line="nodesc{service_class=\"$sc_name\",job_id=\"$current_job\",user=\"$current_user\",nb_nodes=\"$unique_nodes\",status=\"$status\"} 1"
                        echo "$metric_line" >> "$LOG_FILE"
                        all_metrics+="$metric_line"$'\n'
                        echo "DEBUG: Job $current_job user $current_user has $unique_nodes running nodes" >> "$LOG_FILE"
                    fi
...
echo "DEBUG: node_per_job metrics collection completed" >> "$LOG_FILE"
